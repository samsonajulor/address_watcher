...
const bootstrap = async () => {
  // TODO: convert to event driven to ensure functions run in
  // correct orders after releasing the bytestream.
...
const graphiql = async () => {
  ...
  const graphql = spawn('node', ['./scripts/graphiql.mjs']);
  spinner.succeed('[GraphiQL] graphiql started');
  graphql.stdout.on('data', (buffer) => {
...
const express = async () => {
  ...
  spinner.info('[ExpressJS] starting serverjs app');
  server.stdout.on('data', (buffer) => {
    console.log('[ExpressJS]', buffer.toString());
  });
};
